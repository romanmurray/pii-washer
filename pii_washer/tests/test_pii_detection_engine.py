...
from pii_washer.pii_detection_engine import (
    CustomCreditCardRecognizer,
    LabeledIdentifierRecognizer,
    PIIDetectionEngine,
)
...
def test_detect_phone_parentheses_one_exchange(engine):
    """'(555) 123-4567' — the ubiquitous example number. Its exchange starts
    with '1' (not NANP-valid), but users paste this constantly, so the
    parenthesized format must still catch it."""
    text = "Call me at (555) 123-4567 today."
    results = engine.detect(text)
    phones = [r for r in results if r["category"] == "PHONE"]
    assert len(phones) >= 1
    assert "(555) 123-4567" in phones[0]["original_value"]
...
def test_detect_dob_with_context(engine):
    ...
    # A date far from any keyword → surfaces at 0.2 (keywordless behavior).
    text = (
...
def test_no_dob_without_context(engine):
    # Dates without context now surface at 0.2 instead of being dropped.
...
def test_detect_no_pii(engine):
    # "today" and similar words may be classified as DATE_TIME at very low confidence.
...
# =============================================================================
# Name recognizer integration tests
# =============================================================================
...
# =============================================================================
# Phone number pattern hardening tests
# =============================================================================
...
# =============================================================================
# Address pattern hardening tests
# =============================================================================
...
def test_ccn_invalid_luhn_rejected_without_context(engine):
    """Invalid Luhn number with no card context should NOT be detected.
    (Near card context it IS flagged at 0.5 — see TestCardContextFallback.)"""
    text = "Serial: 4111 1111 1111 1112"
    results = engine.detect(text)
    ccns = [r for r in results if r["category"] == "CCN"]
    # The modified last digit breaks Luhn and nothing nearby says "card"
    assert len(ccns) == 0


# =============================================================================
# IP address enhancement tests
# =============================================================================
...
# =============================================================================
# Context filter loosening tests
# =============================================================================
...
# =============================================================================
# Integration tests — new format variants
# =============================================================================
...
# =============================================================================
# Date plausibility filter — digit runs and relative words are not DOBs
# =============================================================================

class TestDatePlausibility:
    def test_digit_run_is_not_a_date(self):
        assert PIIDetectionEngine._plausible_dob("4532 1876 4421") is False

    def test_relative_word_is_not_a_date(self):
        assert PIIDetectionEngine._plausible_dob("today") is False

    def test_real_date_formats_pass(self):
        assert PIIDetectionEngine._plausible_dob("July 9, 1985") is True
        assert PIIDetectionEngine._plausible_dob("09/28") is True
        assert PIIDetectionEngine._plausible_dob("1989") is True
        assert PIIDetectionEngine._plausible_dob("3/14/1989") is True


def test_card_digit_run_not_tagged_dob(engine):
    """A card-formatted digit run must never surface as a DOB detection."""
    text = "Card number: 4532 1876 4421 9087"
    results = engine.detect(text)
    dobs = [r for r in results if r["category"] == "DOB"]
    assert not any("4421" in d["original_value"] for d in dobs)


def test_relative_date_word_not_tagged_dob(engine):
    text = "Thanks for coming in today."
    results = engine.detect(text)
    dobs = [r for r in results if r["category"] == "DOB"]
    assert not any(d["original_value"].strip().lower() == "today" for d in dobs)


# =============================================================================
# Credit card context fallback — non-Luhn numbers near card context
# =============================================================================

class TestCardContextFallback:
    def test_non_luhn_with_card_context_full_span(self):
        rec = CustomCreditCardRecognizer()
        text = "Card number: 4532 1876 4421 9087, expiration 09/28"
        results = rec.analyze(text, ["CREDIT_CARD"])
        assert len(results) == 1
        assert text[results[0].start:results[0].end] == "4532 1876 4421 9087"
        assert results[0].score == pytest.approx(0.5)

    def test_non_luhn_without_context_still_rejected(self):
        rec = CustomCreditCardRecognizer()
        text = "Tracking reference 4532 1876 4421 9087 was logged."
        results = rec.analyze(text, ["CREDIT_CARD"])
        assert results == []

    def test_luhn_valid_keeps_high_confidence(self):
        rec = CustomCreditCardRecognizer()
    ...
    def test_non_luhn_dashed_with_context(self):
        rec = CustomCreditCardRecognizer()
        text = "Credit card on file: 4532-1876-4421-9087"
        results = rec.analyze(text, ["CREDIT_CARD"])
        assert len(results) == 1
        assert text[results[0].start:results[0].end] == "4532-1876-4421-9087"


def test_non_luhn_card_detected_whole_via_engine(engine):
    """End to end: the whole non-Luhn card near card context becomes one CCN."""
    text = "Card number: 4532 1876 4421 9087"
    results = engine.detect(text)
    ccns = [r for r in results if r["category"] == "CCN"]
    assert len(ccns) == 1
    assert ccns[0]["original_value"] == "4532 1876 4421 9087"


# =============================================================================
# Labeled identifier recognizer — "<label>: <code>" catches IDs by their label
# =============================================================================

class TestLabeledIdentifierRecognizer:
    @pytest.mark.parametrize("text,expected", [
        ("Routing number: 071923284", "071923284"),
        ("Account number: 483920174650", "483920174650"),
        ("CVV: 412", "412"),
        ("Driver license number: C938-4176-2201-08", "C938-4176-2201-08"),
        ("Passport number: X58374192", "X58374192"),
        ("Member ID: HLT-48290317-01", "HLT-48290317-01"),
        ("Group number: G284910", "G284910"),
        ("License plate: WKM-4837", "WKM-4837"),
        ("Employee ID: EMP-104928", "EMP-104928"),
        ("Customer ID: CUST-88392017", "CUST-88392017"),
        ("Order Number: ORD-2026-774193", "ORD-2026-774193"),
        ("Policy number: POL-99-1234", "POL-99-1234"),
    ])
    def test_labeled_ids_detected(self, text, expected):
        rec = LabeledIdentifierRecognizer()
        results = rec.analyze(text, ["LABELED_ID"])
        assert len(results) == 1
        assert text[results[0].start:results[0].end] == expected

    def test_value_without_digits_not_matched(self):
        """Labels followed by prose (no digits) are not identifiers."""
        rec = LabeledIdentifierRecognizer()
        results = rec.analyze("Account note: pending review", ["LABELED_ID"])
        assert results == []

    def test_unlabeled_code_not_matched(self):
        """A bare code with no label is too ambiguous to tag."""
        rec = LabeledIdentifierRecognizer()
        results = rec.analyze("See EMP-104928 for details.", ["LABELED_ID"])
        assert results == []


def test_labeled_id_maps_to_id_category(engine):
    text = "Routing number: 071923284"
    results = engine.detect(text)
    ids = [r for r in results if r["category"] == "ID"]
    assert len(ids) == 1
    assert ids[0]["original_value"] == "071923284"


# =============================================================================
# Name newline bleed — NER spans must not cross line breaks
# =============================================================================

def test_name_does_not_bleed_across_newline(engine):
    """spaCy can tag "Michael A. Torres\nDate" as one PERSON span when a
    labeled line follows a name. The span must be trimmed at the newline."""
    text = "Full legal name: Michael A. Torres\nDate of birth: March 14, 1989"
    results = engine.detect(text)
    names = [r for r in results if r["category"] == "NAME"]
    assert names, "The name itself should still be detected"
    for n in names:
        assert "\n" not in n["original_value"]
        assert "Date" not in n["original_value"]
    # Positions must match the trimmed value
    for n in names:
        start = n["positions"][0]["start"]
        end = n["positions"][0]["end"]
        assert text[start:end] == n["original_value"]
